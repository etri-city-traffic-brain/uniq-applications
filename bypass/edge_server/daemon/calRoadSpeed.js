...
const updateRoadInfo = (roadId, speedAvg) => {
  return new Promise((resolve, reject) => {
    ...
    query(
      "edge",
      "UPDATE TB_ROAD SET avg_speed = ?, update_date = ? WHERE id = ?;",
      [speedAvg, updateDate, roadId]
    )
      .then((result) => {
        resolve();
      })
...
const getTrackingIdList = () => {
  return new Promise((resolve, reject) => {
    ...
    query(
      "edge",
      `SELECT uuid, trackingid FROM ${tableName} WHERE regdate >= ? GROUP BY uuid, trackingid;`,
      [searchTime]
    )
      .then((result) => {
...
const getTrackingInfo = (uuid, trackingId) => {
  return new Promise((resolve, reject) => {
    ...
    query(
      "edge",
      `SELECT * FROM ${tableName} WHERE regdate >= ? AND uuid = ? AND trackingid = ? ORDER BY regdate DESC;`,
      [searchTime, uuid, trackingId]
    )
      .then((result) => {
        resolve(result);
      })
        ...
        await getTrackingInfo(trackingId.uuid, trackingId.trackingid)
          .then((result) => {
            for (var idx3 in result) {
              ...
              if (
                pointInPolygon(
                  JSON.parse(tracking1.coordinate),
                  JSON.parse(roadInfo.coordinate)
                ) == true &&
                pointInPolygon(
                  JSON.parse(tracking2.coordinate),
                  JSON.parse(roadInfo.coordinate)
                ) == true
              ) {
                var dist1 = getDistance(
                  JSON.parse(tracking1.coordinate),
                  [roadInfo.lat, roadInfo.lng],
                  2
                );
